...
with open('articles.csv', 'r', encoding='utf-8') as f:
    ...
    for row in reader:
        ...
        if row.get('Status') == 'Done' and title and content and kw not in seen_keywords:
            ...
            # Clean content (remove markdown backticks if any, ensure HTML)
...
constants_content = "export const BLOG_POSTS = [\n"
for a in top_10:
    # Escape quotes
    title_safe = a['title'].replace("'", "\\'").replace('"', '\\"')
    excerpt_safe = a['excerpt'].replace("'", "\\'").replace('"', '\\"')
    # Use template literals for content to handle newlines easily
...
# Also include other constants if any. 
# Looking at previous logs, constants.ts only exported BLOG_POSTS initially. Let's rewrite constants.ts completely.
with open('constants.ts', 'w', encoding='utf-8') as f:
    f.write(constants_content)

print(f"Generated constants.ts with top 10 articles")
